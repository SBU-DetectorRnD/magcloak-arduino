...
float V_A0_threshold = 2.8; //Rtop2 is cold if V0 is beyond this threshold
float V_A1_threshold = 3.12; //Rbottom2 is cold if V1 is beyond this threshold
...
//  Serial.println(voltage1); //in case you want to read out the voltage
  ...
  if (voltage0 > V_A0_threshold && voltage1>V_A1_threshold){ //this signifies that both resistors are at liquid nitrogen temperature
    digitalWrite(relay_pin,LOW); //make the switch open
  }
    ...
    digitalWrite(relay_pin, HIGH); //make the switch close
  }
